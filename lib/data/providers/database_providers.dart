...
final appLocaleProvider = FutureProvider<Locale?>((ref) async {
  ...
  if (localeString == 'system') return null;
...
/// 토픽별 학습 기록
final studyRecordsByTopicProvider =
    FutureProvider.family<List<StudyRecord>, String>(
  (ref, topicId) async {
    final dao = ref.watch(studyRecordsDaoProvider);
    return dao.getByTopicId(topicId);
  },
);

/// 레벨별 학습 기록
final studyRecordsByLevelProvider =
    FutureProvider.family<List<StudyRecord>, String>(
  (ref, levelId) async {
    final dao = ref.watch(studyRecordsDaoProvider);
    return dao.getByLevelId(levelId);
  },
);
...
final uncorrectedWrongAnswerIdsProvider =
  ...
  final wrongAnswers =
      await ref.watch(uncorrectedWrongAnswersProvider.future);
...
/// 토픽별 오답
final wrongAnswersByTopicProvider =
...
/// 레벨별 오답
final wrongAnswersByLevelProvider =
    FutureProvider.family<List<WrongAnswer>, String>(
  (ref, levelId) async {
    final dao = ref.watch(wrongAnswersDaoProvider);
    return dao.getByLevelId(levelId);
